...
Re-verified against provider documentation on 2026-08-23. The production read-only validator requires explicit plan attestations and fails if any application gate is unmet.

| Provider | Verified included behavior | Internal fail-closed control |
| --- | --- | --- |
| Supabase Free | $0; 500 MB database; 1 GB file storage; 5 GB egress and cached egress; no automatic backups; projects can pause after one inactive week; two active projects. Leaked-password protection is not included on Free. | Profile constraints cap database at 400 MB and storage at 800 MB. Retention jobs remove bounded payloads. Production gate requires `RADAR_SUPABASE_PLAN=free`. |
