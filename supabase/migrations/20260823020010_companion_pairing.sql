...
create or replace function public.authenticate_device_token (
  p_token_hash text
)
  returns table (
    token_id     bigint,
...
create or replace function public.claim_next_companion_application (
  p_token_hash text
)
  returns table (
    application_id uuid,
...
create or replace function public.record_companion_event (
  p_token_hash     text,
  p_application_id uuid,
  p_event_type     text,
  p_detail         jsonb
)
  returns public.application_state
  language plpgsql
  set search_path to ''
  AS $function$
...
revoke all on function "public"."authenticate_device_token"(text) from public;

grant execute on function "public"."authenticate_device_token"(text) to "postgres", "service_role";

revoke all on function "public"."claim_next_companion_application"(text) from public;

grant execute on function "public"."claim_next_companion_application"(text) to "postgres", "service_role";

revoke all on function "public"."consume_device_pairing"(text, text) from public;

grant execute on function "public"."consume_device_pairing"(text, text) to "postgres", "service_role";

revoke all on function "public"."create_device_pairing"(text, text) from public;

grant execute on function "public"."create_device_pairing"(text, text) to "authenticated", "postgres";

revoke all on function "public"."record_companion_event"(text, uuid, text, jsonb) from public;

grant execute on function "public"."record_companion_event"(text, uuid, text, jsonb) to "postgres", "service_role";

revoke all on function "public"."revoke_device_token"(bigint) from public;

grant execute on function "public"."revoke_device_token"(bigint) to "authenticated", "postgres";
