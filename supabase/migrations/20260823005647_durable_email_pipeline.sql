...
begin
  ...
  if (select count(*) from public.email_deliveries delivery
    ...
    update public.email_outbox set next_attempt_at = date_trunc('month', now()) + interval '1 month', updated_at = now()
    where owner_id = p_owner_id and state in ('pending', 'retry_wait') and next_attempt_at <= now();
    return;
  end if;
  if (select count(*) from public.email_deliveries delivery
    ...
    update public.email_outbox set next_attempt_at = date_trunc('day', now()) + interval '1 day', updated_at = now()
    where owner_id = p_owner_id and state in ('pending', 'retry_wait') and next_attempt_at <= now();
    return;
  end if;
...
begin
  update public.email_outbox outbox set
    state = case when p_retryable and outbox.attempts < outbox.max_attempts then 'retry_wait' else 'failed' end,
...
create or replace function public.record_resend_webhook (
  p_owner_id           uuid,
  p_event_id           text,
  p_event_type         text,
  p_resend_message_id  text,
...
create or replace function public.upsert_discovered_job_with_alert (
  p_owner_id              uuid,
...
grant execute on function "public"."claim_email_outbox"(uuid, integer) to "postgres", "service_role";

revoke all on function "public"."record_email_failure"(bigint, boolean, text) from public;

grant execute on function "public"."record_email_failure"(bigint, boolean, text) to "postgres", "service_role";

revoke all on function "public"."record_email_send"(bigint, text) from public;

grant execute on function "public"."record_email_send"(bigint, text) to "postgres", "service_role";

revoke all on function "public"."record_resend_webhook"(uuid, text, text, text, text, jsonb) from public;

grant execute on function "public"."record_resend_webhook"(uuid, text, text, text, text, jsonb) to "postgres", "service_role";

revoke all
  on function "public"."upsert_discovered_job_with_alert"(uuid, bigint, text, text, text, text, text, text, text, text, text, timestamp with time zone, timestamp
    with time zone, text, public.job_state, smallint, smallint, smallint, smallint, smallint, jsonb, bigint, text)
  from public;
