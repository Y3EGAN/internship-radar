...
export default async function JobsPage({ searchParams }: { searchParams: Promise<JobSearch> }) {
  ...
  let query = client
    .from("jobs")
    .select("id,title,employer_name,location_text,state,preliminary_score,posted_at,discovered_at,saved_at,applied_at,companies(name)")
