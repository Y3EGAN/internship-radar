...
export default async function JobPage({ params }: { readonly params: Promise<{ id:string }> }) {
  ...
  const { data } = await client.from("jobs").select("id,title,description,location_text,state,preliminary_score,canonical_url,posted_at,closes_at,saved_at,applied_at,companies(name)").eq("id",id).maybeSingle();
  if (!data) notFound();
